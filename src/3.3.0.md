...
<a id="download" href="https://github.com/atuttle/Taffy/archive/v3.3.0.zip">Download Taffy 3.3.0.zip</a>
...
### Special case request content types

Automatic support for special headers has been added for CSP reporting and ndJSON.

- [CSP reporting](https://websec.be/blog/cspreporting/) uses an `application/csp-report` content type and posts a standard JSON payload.
- [ndJSON](http://ndjson.org/) uses an `application/x-ndjson` content type and posts newline-delimited JSON payloads. Some data providers don't pass the correct headers, so a detection routine is used.
...
##### Support for `Access-Control-Expose-Headers` \([#377](https://github.com/atuttle/Taffy/issues/377)\)

See the details at [Mozilla](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Access-Control-Expose-Headers).

##### Support for flexibility in cacheKey generation \([#380](https://github.com/atuttle/Taffy/issues/380)\)
